...
model = TwoStageCNNGeometric(use_cuda=use_cuda, return_correlation=True, **arg_groups['model'])
...
if args.model_aff != '':
    ...
    for name, param in model.FeatureExtraction.state_dict().items():
        model.FeatureExtraction.state_dict()[name].copy_(
            checkpoint_aff['state_dict']['FeatureExtraction.' + name]
        )
    for name, param in model.FeatureRegression.state_dict().items():
        model.FeatureRegression.state_dict()[name].copy_(
            checkpoint_aff['state_dict']['FeatureRegression.' + name]
        )
...
for name, param in model.FeatureExtraction.named_parameters():
    ...
    if args.train_fe and name.find('bn') != -1 and np.sum(
        [name.find(x) != -1 for x in args.fe_finetune_params]
    ):
...
def inlier_score_function(
    theta_aff, theta_aff_tps, corr_aff, corr_aff_tps, minimize_outliers=False
):
...
def loss_fun(batch):
    ...
    inlier_score_pos = inlier_score_function(theta_aff, theta_aff_tps, corr_aff, corr_aff_tps)
...
dataset = ImagePairDataset(
    csv_file=os.path.join(args.dataset_csv_path, 'train_pairs.csv'),
...
dataset_eval = PFPascalDataset(
    csv_file=os.path.join(args.eval_dataset_path, 'val_pairs_pf_pascal.csv'),
...
dataloader_eval = DataLoader(dataset_eval, batch_size=8, shuffle=False, num_workers=4)
...
checkpoint_name = os.path.join(
    args.result_model_dir, args.result_model_fn + checkpoint_suffix + '.pth.tar'
)
...
def process_epoch(
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        if batch_idx % log_interval == 0:
            print(
                mode.capitalize() + ' Epoch: {} [{}/{} ({:.0f}%)]\t\tLoss: {:.6f}'.format(
                    epoch, batch_idx, len(dataloader), 100. * batch_idx / len(dataloader), loss_np
                )
            )
...
stats = compute_metric(
    metric, model, dataset_eval, dataloader_eval, batch_tnf, 8, two_stage, do_aff, do_tps, args
)
...
for epoch in range(1, args.num_epochs + 1):
    ...
    train_loss[epoch - 1] = process_epoch(
        'train', epoch, model, loss_fun, optimizer, dataloader, batch_tnf, log_interval=1
    )
    model.eval()
    stats = compute_metric(
        metric, model, dataset_eval, dataloader_eval, batch_tnf, 8, two_stage, do_aff, do_tps, args
