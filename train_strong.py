...
model = CNNGeometric(use_cuda=use_cuda, output_dim=cnn_output_dim, **arg_groups['model'])
...
)  # don't change num_workers, as they copy the rnd seed
...
pair_generation_tnf = SynthPairTnf(
    geometric_model=args.geometric_model, output_size=cnn_image_size, use_cuda=use_cuda
)
...
for epoch in range(1, args.num_epochs + 1):
    ...
    train_loss[epoch - 1] = process_epoch(
        'train', epoch, model, loss, optimizer, dataloader, pair_generation_tnf, log_interval=100
    )
