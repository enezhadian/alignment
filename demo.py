...
parser.add_argument(
    '--model',
    type=str,
    default='trained_models/weakalign_resnet101_affine_tps.pth.tar',
...
parser.add_argument(
    '--pf-path', type=str, default='datasets/proposal-flow-pascal', help='Path to PF dataset'
)
parser.add_argument(
...
model = TwoStageCNNGeometric(
    use_cuda=use_cuda, return_correlation=False, feature_extraction_cnn=args.feature_extraction_cnn
)
...
def affTpsTnf(source_image, theta_aff, theta_aff_tps, use_cuda=use_cuda):
    ...
    sampling_grid = tpstnf(
        image_batch=source_image, theta_batch=theta_aff_tps, return_sampling_grid=True
    )[1]
...
for i, batch in enumerate(dataloader):
    ...
    warped_image_aff = warped_image_aff.data.squeeze(0).transpose(0, 1).transpose(1,
                                                                                  2).cpu().numpy()

    warped_image_aff_tps = normalize_image(warped_image_aff_tps, forward=False)
    warped_image_aff_tps = warped_image_aff_tps.data.squeeze(0).transpose(0, 1).transpose(
        1, 2
    ).cpu().numpy()

    # check if display is available
    exit_val = os.system(
        'python -c "import matplotlib.pyplot as plt;plt.figure()"  > /dev/null 2>&1'
    )
