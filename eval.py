...
if two_stage:
    model = TwoStageCNNGeometric(use_cuda=use_cuda, **arg_groups['model'])
if not two_stage and do_aff:
    model = CNNGeometric(use_cuda=use_cuda, output_dim=6, **arg_groups['model'])

if not two_stage and do_tps:
    model_tps = CNNGeometric(use_cuda=use_cuda, output_dim=18, **arg_groups['model'])
...
stats = compute_metric(
    metric, model, dataset, dataloader, batch_tnf, batch_size, two_stage, do_aff, do_tps, args
)
