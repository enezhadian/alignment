...
class ComposedGeometricTnf(object):
    ...
    def __call__(self, image_batch, theta_aff, theta_aff_tps, use_cuda=True):
        ...
        sampling_grid_aff_tps = self.tpsTnf(
            image_batch=None,
        ...
        in_bound_mask_aff = (
            (sampling_grid_aff[:, :, :, 0] > -1) * (sampling_grid_aff[:, :, :, 0] < 1) *
            (sampling_grid_aff[:, :, :, 1] > -1) * (sampling_grid_aff[:, :, :, 1] < 1)
        ).unsqueeze(3)
        ...
        sampling_grid_aff_tps_comp = F.grid_sample(
            sampling_grid_aff.transpose(2, 3).transpose(1, 2), sampling_grid_aff_tps
        ).transpose(1, 2).transpose(2, 3)

        # put 1e10 value in region out of bounds of sampling_grid_aff_tps_comp
        in_bound_mask_aff_tps = (
            (sampling_grid_aff_tps[:, :, :, 0] > -1) * (sampling_grid_aff_tps[:, :, :, 0] < 1) *
            (sampling_grid_aff_tps[:, :, :, 1] > -1) * (sampling_grid_aff_tps[:, :, :, 1] < 1)
        ).unsqueeze(3)
...
class GeometricTnf(object):
    ...
    def __call__(
        ...
        if (out_h is not None and
            out_w is not None) and (out_h != self.out_h or out_w != self.out_w):
...
class SynthPairTnf(object):
    ...
    def __init__(
        ...
        self.rescalingTnf = GeometricTnf(
            'affine', out_h=self.out_h, out_w=self.out_w, use_cuda=self.use_cuda
        )
        self.geometricTnf = GeometricTnf(
            geometric_model, out_h=self.out_h, out_w=self.out_w, use_cuda=self.use_cuda
        )
    ...
    def __call__(self, batch):
        ...
        if self.supervision == 'strong':
            ...
            source_image = torch.cat(
                (
                    torch.index_select(cropped_image_batch, 0, pos_batch_idx),
                    torch.index_select(cropped_image_batch, 0, pos_batch_idx)
                ), 0
            )
            target_image = torch.cat(
                (
                    torch.index_select(warped_image_batch, 0, pos_batch_idx),
                    torch.index_select(cropped_image_batch, 0, neg_batch_idx)
                ), 0
            )
            return {
    ...
    def symmetricImagePad(self, image_batch, padding_factor):
        ...
        image_batch = torch.cat(
            (
                image_batch.index_select(3, idx_pad_left), image_batch,
                image_batch.index_select(3, idx_pad_right)
            ), 3
        )
        image_batch = torch.cat(
            (
                image_batch.index_select(2, idx_pad_top), image_batch,
...
class SynthTwoStageTnf(SynthPairTnf):
    ...
    def __init__(
        ...
        self.geometricTnf = ComposedGeometricTnf(
            padding_crop_factor=padding_factor * crop_factor, use_cuda=self.use_cuda
        )
    ...
    def __call__(self, batch):
        ...
        # get transformed image
        warped_image_batch = self.geometricTnf(
            image_batch=image_batch, theta_aff=theta_aff, theta_aff_tps=theta_tps
        )
...
class SynthTwoStageTwoPairTnf(SynthPairTnf):
    ...
    def __init__(
        ...
        self.affTnf = GeometricTnf(
            geometric_model='affine', out_h=self.out_h, out_w=self.out_w, use_cuda=self.use_cuda
        )
        self.tpsTnf = GeometricTnf(
            geometric_model='tps', out_h=self.out_h, out_w=self.out_w, use_cuda=self.use_cuda
        )
    ...
    def __call__(self, batch):
        ...
        # get transformed image
        target_image_tps = self.geometricTnf(
            image_batch=image_batch, theta_aff=theta_aff, theta_aff_tps=theta_tps
        )

        target_image_aff = self.affTnf(
            image_batch=image_batch,
        ...
        source_image_tps = self.affTnf(
            image_batch=cropped_image_batch,
            theta_batch=theta_aff,
            padding_factor=1.0,
            crop_factor=1.0
        )
        ...
        return {
            'source_image_aff': cropped_image_batch,
...
class SynthTwoPairTnf(SynthPairTnf):
    ...
    def __call__(self, batch):
        ...
        return {
            'source_image': cropped_image_batch,
            'target_image_aff': warped_image_aff,
            'target_image_tps': warped_image_tps,
            'theta_GT_aff': theta_aff,
            'theta_GT_tps': theta_tps
        }
...
class TpsGridGen(Module):
    ...
    def compute_L_inverse(self, X, Y):
        ...
        Xmat = X.expand(N, N)
        Ymat = Y.expand(N, N)
        P_dist_squared = torch.pow(Xmat - Xmat.transpose(0, 1),
                                   2) + torch.pow(Ymat - Ymat.transpose(0, 1), 2)
