...
def compute_metric(
    ...
    if metric == 'dist':
        ...
        metrics = ['intersection_over_union', 'label_transfer_accuracy', 'localization_error']
    ...
    for i, batch in enumerate(dataloader):
        ...
        if metric == 'inlier_count':
            stats = inlier_count(
                batch, batch_start_idx, theta_aff, theta_tps, theta_aff_tps, corr_aff, corr_aff_tps,
                stats, args
            )
        elif metric_fun is not None:
            stats = metric_fun(
                batch, batch_start_idx, theta_aff, theta_tps, theta_aff_tps, stats, args
            )
...
def point_dist_metric(
    batch, batch_start_idx, theta_aff, theta_tps, theta_aff_tps, stats, args, use_cuda=True
):
...
def inlier_count(
    ...
    inliersComposed = TwoStageWeakInlierCount(
        use_cuda=torch.cuda.is_available(), dilation_filter=0, normalize_inlier_count=True
    )
    inliers_comp = inliersComposed(
        matches=corr_aff, theta_aff=theta_aff, theta_aff_tps=theta_aff_tps
    )
    current_batch_size = batch['source_im_size'].size(0)
    indices = range(batch_start_idx, batch_start_idx + current_batch_size)
...
def pascal_parts_metrics(
    ...
    for b in range(batch_size):
        ...
        if batch['keypoint_A'][b].size != 0:
            ...
            batch_b = {
                'source_im_size': batch['source_im_size'][b, :].unsqueeze(0),
            ...
            args.pck_alpha = 0.05
            stats = pck_metric(
                batch_b, idx, theta_aff[b, :].unsqueeze(0) if do_aff else None,
                theta_tps[b, :].unsqueeze(0) if do_tps else None,
                theta_aff_tps[b, :].unsqueeze(0) if do_aff_tps else None, stats, args, use_cuda
            )
        else:
            if do_aff:
                stats['aff']['pck'][idx] = -1
        ...
        source_mask = Variable(
            torch.FloatTensor(batch['part_A'][b].astype(np.float32)
                             ).unsqueeze(0).transpose(2, 3).transpose(1, 2)
        )
        target_mask = Variable(
            torch.FloatTensor(batch['part_B'][b].astype(np.float32)
                             ).unsqueeze(0).transpose(2, 3).transpose(1, 2)
        )
        ...
        if do_aff:
            ...
            stats['aff']['intersection_over_union'][idx] = intersection_over_union(
                warped_mask_aff, target_mask
            )
        if do_tps:
            warped_mask_tps = F.grid_sample(source_mask, grid_tps)
            stats['tps']['intersection_over_union'][idx] = intersection_over_union(
                warped_mask_tps, target_mask
            )
        if do_aff_tps:
            warped_mask_aff_tps = F.grid_sample(source_mask, grid_aff_tps)
            stats['aff_tps']['intersection_over_union'][idx] = intersection_over_union(
                warped_mask_aff_tps, target_mask
            )
...
def area_metrics(
    ...
    for b in range(batch_size):
        ...
        if do_aff:
            ...
            stats['aff']['label_transfer_accuracy'][idx] = label_transfer_accuracy(
                warped_mask_aff, target_mask
            )
        ...
        if do_tps:
            ...
            stats['tps']['label_transfer_accuracy'][idx] = label_transfer_accuracy(
                warped_mask_tps, target_mask
            )
        ...
        if do_aff_tps:
            ...
            stats['aff_tps']['label_transfer_accuracy'][idx] = label_transfer_accuracy(
                warped_mask_aff_tps, target_mask
            )
...
def poly_str_to_mask(poly_x_str, poly_y_str, out_h, out_w, use_cuda=True):
    ...
    mask_np = poly_to_mask(
        vertex_col_coords=polygon_x, vertex_row_coords=polygon_y, shape=[out_h, out_w]
    )
...
def intersection_over_union(warped_mask, target_mask):
    relative_part_weight = torch.sum(torch.sum(target_mask.data.gt(0.5).float(), 2, True), 3,
                                     True) / torch.sum(target_mask.data.gt(0.5).float())
    part_iou = torch.sum(
